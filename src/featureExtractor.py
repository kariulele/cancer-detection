...
def pretrain_desc(sto, fun_img_to_desc):
    ...
    cancer_desc = [fun_img_to_desc(elm.content) for elm in rdn_cancer_img]
    no_cancer_desc = [fun_img_to_desc(elm.content) for elm in rdn_nocancer_img]
    ...
    pca_transform = eigvecs[:, perm[64:] ]  # eigenvectors for the 61 last eigenvalues
...
def compute_descriptors(sto, train_mean, pca_transform, kmeans, fun_img_to_desc):
    ...
    for ii  in range(sto.size()):
        ...
        desc = fun_img_to_desc(img.content)
        ...
        descr_hist = np.histogram(clabels, np.arange(513))[0]
...
def compute_all_descriptor(sto, fun_img_to_desc):
    kmeans, train_mean, pca_transform = pretrain_desc(sto,fun_img_to_desc)
    descs = compute_descriptors(sto, train_mean, pca_transform,kmeans, fun_img_to_desc)
    return descs

def make_train_and_test_list(sto, test_proportion, fun_img_to_desc):
    descs = compute_all_descriptor(sto, fun_img_to_desc)
